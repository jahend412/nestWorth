...
export const getAllTransactions = catchAsync(async (req, res, _next) => {
...
export const createTransaction = catchAsync(async (req, res, next) => {
  ...
  if (!amount || !description || !category || !type) {
    return next(
      new AppError("Amount, description, category, and type are required", 400)
    );
  }
...
export const getTransactionSummary = catchAsync(async (req, res, _next) => {
  const { userId } = req.user;
  const transactions = await Transaction.findByUserId(userId);
...
class TransactionController {
  ...
  static getTransactionById = getTransactionById;
  static createTransaction = createTransaction;
  static updateTransaction = updateTransaction;
  static deleteTransaction = deleteTransaction;
  static getTransactionSummary = getTransactionSummary;
}
